...
- IP-Based: ${result.details.isIpBased ? 'Yes' : 'No'}
- Suspicious TLD: ${result.details.suspiciousTLD ? 'Yes' : 'No'}
- Phishing Keywords: ${result.details.hasPhishingKeywords ? 'Detected' : 'None'}
- Excessive Subdomains: ${result.details.tooManySubdomains ? 'Yes' : 'No'}

Generated by DeepCheck - Advanced Security Analysis Tool
        `.trim();

        const whatsappText = `🔒 *DeepCheck URL Security Audit*
...
🔧 *Technical Findings:*
• Protocol: ${result.details.protocol}
• HTTPS: ${result.details.hasHttps ? '✅ Secure' : '❌ Insecure'}
• IP-Based: ${result.details.isIpBased ? '⚠️ Yes' : '✅ No'}
• Suspicious TLD: ${result.details.suspiciousTLD ? '⚠️ Detected' : '✅ None'}
• Phishing Keywords: ${result.details.hasPhishingKeywords ? '⚠️ Detected' : '✅ None'}
• Excessive Subdomains: ${result.details.tooManySubdomains ? '⚠️ Yes' : '✅ No'}
...
Generated by DeepCheck - Advanced Security Analysis Tool`;
        ...
        document.getElementById('share-telegram')?.addEventListener('click', () => {
            const telegramUrl = `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(`URL Security Report: ${result.classification}`)}`;
        ...
        document.getElementById('share-twitter')?.addEventListener('click', () => {
            const twitterText = `🔒 Just analyzed a URL with DeepCheck:\n${url}\n📊 Classification: ${result.classification}\n⚠️ Risk Score: ${result.riskScore}/100\n\n#Security #DeepCheck`;
        ...
        document.getElementById('share-facebook')?.addEventListener('click', () => {
            const facebookUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}&quote=${encodeURIComponent(`DeepCheck URL Security Report: ${result.classification}`)}`;
            ...
            <div className="w-full px-6 pb-2 flex flex-col items-center shrink-0 z-10 bg-white shadow-sm relative">
                ...
                <div className="w-full max-w-sm flex flex-col items-center mb-3">
                    <div className="w-full flex justify-between text-xs font-bold uppercase tracking-wider mb-1 px-1 opacity-70">
                        ...
                        <span>{result.riskScore > 50 ? result.riskScore : (100 - result.riskScore)}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                            ...
                            style={{ width: `${result.riskScore > 50 ? result.riskScore : (100 - result.riskScore)}%` }}
                        ></div>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full max-w-2xl mx-auto flex flex-col gap-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-white border border-slate-100 rounded-lg p-3 flex items-center justify-between shadow-sm">
                            ...
                            <span className={`${colors.text} font-semibold text-sm`}>{result.details.protocol}</span>
                        </div>
                        <div className="bg-white border border-slate-100 rounded-lg p-3 flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-3">
                                <Icon name={result.details.hasHttps ? "check_circle" : "dangerous"} className={`text-lg ${result.details.hasHttps ? 'text-green-600' : 'text-red-600'}`} />
                                <span className="text-slate-700 font-medium text-sm">HTTPS</span>
                            </div>
                            <span className={`${result.details.hasHttps ? 'text-green-600' : 'text-red-600'} font-semibold text-sm`}>
                                {result.details.hasHttps ? 'Verified' : 'Missing'}
                            </span>
                        </div>
                        <div className="bg-white border border-slate-100 rounded-lg p-3 flex items-center justify-between shadow-sm">
                            <div className="flex items-center gap-3">
                                <Icon name={result.details.hasPhishingKeywords ? "dangerous" : "check_circle"} className={`text-lg ${result.details.hasPhishingKeywords ? 'text-red-600' : 'text-green-600'}`} />
                                <span className="text-slate-700 font-medium text-sm">Phishing Risk</span>
                            </div>
                            <span className={`${result.details.hasPhishingKeywords ? 'text-red-600' : 'text-green-600'} font-semibold text-sm`}>
                                {result.details.hasPhishingKeywords ? 'Detected' : 'None'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="bg-white border border-slate-100 rounded-lg p-3 flex items-center gap-3 shadow-sm">
                            <div className={`w-10 h-10 rounded-lg ${colors.bg} flex items-center justify-center shrink-0`}>
                                <Icon name={result.classification === 'Legitimate' ? 'check_circle' : result.classification === 'Suspicious' ? 'warning' : 'dangerous'} className={`text-xl ${colors.icon}`} />
                            </div>
